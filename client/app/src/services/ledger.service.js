;(function () {
  ...
  function LedgerService ($q, $http, $timeout, storageService, networkService) {
    ...
    const ripajs = require(_path.resolve(__dirname, '../node_modules/ripajs'))
    ...
    function getBip44Accounts (slip44) {
      ...
      async.whilst(
        ...
        (next) => {
          ...
          if (result.address) {
            result.address = ripajs.crypto.getAddress(result.publicKey)
    ...
    function recoverBip44Accounts (backupLedgerPassphrase, slip44) {
      const hdnode = ripajs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase))
    ...
    function signTransaction (path, transaction) {
      return new Promise((resolve, reject) => {
        ...
        ipcRenderer.send('ledger', {
          ...
          data: ripajs.crypto.getBytes(transaction, true, true).toString('hex'),
          path
        })
      })
    }
